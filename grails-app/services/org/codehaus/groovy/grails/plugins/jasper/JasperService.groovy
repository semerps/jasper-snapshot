...
class JasperService {
	...
	List<JasperReportParameter> getReportParameters(name, locale) {
		...
		List<JasperReportParameter> result = new LinkedList<JasperReportParameter>()
		
		for(JRParameter param : params) {
		  if(!param.isSystemDefined() && param.isForPrompting()){
			 result.add(new JasperReportParameter(name : param.getName(), description : param.getDescription(), nestedType: param.getNestedType(), nestedTypeName: param.getNestedTypeName(), valueClass: param.getValueClass(), valueClassName: param.getValueClassName() ))
		  }
		}
